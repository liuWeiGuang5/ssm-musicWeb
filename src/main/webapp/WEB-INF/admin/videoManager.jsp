...
<body>
					...
					<div class="col-md-12">
						...
						<div class="panel panel-default">
							...
							<div class="panel-heading">
								...
								<form class="form-inline" action="<%=basePath %>/video/toVideoManager" method="post" style="float:right;margin-left: 10px">
									<div class="form-group">
										<label for="exampleInputName2"><font size="2">视频名称</font></label>
										<input type="text" name="videoname" value="${videoname}"  class="form-control" id="exampleInputName2">
									</div>
									<div class="form-group">
										<label for="exampleInputEmail2"><font size="2">视频主角</font></label>
										<input type="text" name="videoauthor" value="${videoauthor}" class="form-control" id="exampleInputEmail2">
									</div>
								...
								<div class="table-responsive">
									<table class="table table-striped table-bordered table-hover dataTables-example "
										...
										<thead>
											<tr>
												...
												<th>修改</th>
												<th>删除</th>
												<th><input type="button" value="全选" onclick="cheall('checkbox')"></th>
											</tr>
										</thead>
										<tbody>
											<c:if test="${not empty info.list }">
												<c:forEach var="m" items="${info.list }">
													<tr>
														...
														<td>${m.videoname }</td>
														<td>${m.videoauthor }</td>
														<td><img src="<%=imgbasePath%>/musicfile/${m.videophoto }" width="100px" height="50px"></td>
														<td>${m.videourl }</td>
														...
														<td><input type="checkbox" name="checkbox" value="${m.videoid}"></td>
													</tr>
												</c:forEach>
											</c:if>
										</tbody>
									...
									<table class="table table-bordered table-hover definewidth m10" >
										<tr><th colspan="5">  <div class="inline pull-right page">
											<a href='<%=basePath%>video/toVideoManager?pageNO=1&videoname=${videoname}&videoauthor=${videoauthor}' >第一页</a>
											<c:if test="${info.pageNum!=1}">
												<a href='<%=basePath%>video/toVideoManager?pageNO=${info.prePage}&videoname=${videoname}&videoauthor=${videoauthor}'>上一页</a>
											</c:if>
											<c:if test="${info.pageNum==1}">
												<a href='<%=basePath%>video/toVideoManager?pageNO=1&videoname=${videoname}&videoauthor=${videoauthor}'>上一页</a>
											</c:if>

											<c:if test="${info.pageNum==info.pages}">
												<a href='<%=basePath%>video/toVideoManager?pageNO=${info.pages}&videoname=${videoname}&videoauthor=${videoauthor}' >下一页</a>
											</c:if>
											<c:if test="${info.pageNum!=info.pages}">
												<a href='<%=basePath%>video/toVideoManager?pageNO=${info.nextPage}&videoname=${videoname}&videoauthor=${videoauthor}' >下一页</a>
											</c:if>
											<a href='<%=basePath%>video/toVideoManager?pageNO=${info.pages}&videoname=${videoname}&videoauthor=${videoauthor}' >最后一页</a>
							...
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
										...
										<!-- 表单嵌套表格：规范表单格式 -->
										<form action="insertVideo.html" id="form" method="post" role="form" enctype="multipart/form-data">
											<input type="hidden" id="op" name="op">
											<input type="hidden" id="vid" name="videoid">
											<div class="form-group">
	...
	<script type="text/javascript">
		...
		function deletes(){
			...
			for(var i=0;i<objNameList.length;i++){
				if (objNameList[i].checked){
					videoId.push(objNameList[i].value)
				}
			}
			...
			$.post("<%=basePath %>video/delByIdAll",{'videos':videoId},function(data){
				alert("删除成功");
		...
		//导出数据
		function expExl(){
			window.location.href="<%=basePath%>video/expExl";
		}
